"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserController = void 0;
var class_validator_1 = require("class-validator");
var data_source_1 = require("../data-source");
var User_1 = require("../entity/User");
var UserController = /** @class */ (function () {
    function UserController() {
    }
    var _a;
    _a = UserController;
    UserController.newUser = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var _b, email, password, user, validationOpt, errors, userRepository, error_1;
        return __generator(_a, function (_c) {
            switch (_c.label) {
                case 0:
                    _b = req.body, email = _b.email, password = _b.password;
                    user = new User_1.User();
                    user.email = email;
                    user.password = password;
                    validationOpt = {
                        validationError: { target: false, value: false },
                    };
                    return [4 /*yield*/, (0, class_validator_1.validate)(user, validationOpt)];
                case 1:
                    errors = _c.sent();
                    if (errors.length > 0) {
                        return [2 /*return*/, res.status(400).send(errors)];
                    }
                    userRepository = data_source_1.AppDataSource.getRepository(User_1.User);
                    _c.label = 2;
                case 2:
                    _c.trys.push([2, 4, , 5]);
                    user.hashPassword(); //encriptamos el password
                    return [4 /*yield*/, userRepository.save(user)];
                case 3:
                    _c.sent(); //guardamos el usuario
                    return [3 /*break*/, 5];
                case 4:
                    error_1 = _c.sent();
                    return [2 /*return*/, res.status(500).json({
                            message: "Error creating user",
                            error: error_1.message
                        })];
                case 5: return [2 /*return*/, res.status(201).json({
                        message: "User created",
                    })];
            }
        });
    }); };
    UserController.getUsers = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var userRepository, users, error_2;
        return __generator(_a, function (_b) {
            switch (_b.label) {
                case 0:
                    userRepository = data_source_1.AppDataSource.getRepository(User_1.User);
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, userRepository.find()];
                case 2:
                    users = _b.sent();
                    return [3 /*break*/, 4];
                case 3:
                    error_2 = _b.sent();
                    return [2 /*return*/, res.status(500).json({
                            message: "Error getting users",
                            error: error_2.message,
                        })];
                case 4:
                    if (users.length > 0) {
                        return [2 /*return*/, res.send(users)];
                    }
                    else {
                        return [2 /*return*/, res.status(404).json({
                                message: "No users found",
                            })];
                    }
                    return [2 /*return*/];
            }
        });
    }); };
    UserController.getByid = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var id, userRepository, user, error_3;
        return __generator(_a, function (_b) {
            switch (_b.label) {
                case 0:
                    id = req.params.id;
                    userRepository = data_source_1.AppDataSource.getRepository(User_1.User);
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, userRepository.findOneByOrFail({ id: parseInt(id) })];
                case 2:
                    user = _b.sent();
                    res.send(user);
                    return [3 /*break*/, 4];
                case 3:
                    error_3 = _b.sent();
                    res.status(404).json({
                        message: "User not found",
                        error: error_3.message,
                    });
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/];
            }
        });
    }); };
    UserController.delete = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var id, userRepository, user, error_4, error_5;
        return __generator(_a, function (_b) {
            switch (_b.label) {
                case 0:
                    id = req.params.id;
                    userRepository = data_source_1.AppDataSource.getRepository(User_1.User);
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, userRepository.findOneByOrFail({ id: parseInt(id) })];
                case 2:
                    user = _b.sent();
                    return [3 /*break*/, 4];
                case 3:
                    error_4 = _b.sent();
                    return [2 /*return*/, res.status(404).json({
                            message: "User not found",
                            error: error_4.message,
                        })];
                case 4:
                    _b.trys.push([4, 6, , 7]);
                    return [4 /*yield*/, userRepository.delete(id)];
                case 5:
                    _b.sent();
                    res.status(200).json({
                        message: "User deleted",
                    });
                    return [3 /*break*/, 7];
                case 6:
                    error_5 = _b.sent();
                    res.status(500).json({
                        message: "Error deleting user",
                        error: error_5.message
                    });
                    return [3 /*break*/, 7];
                case 7: return [2 /*return*/];
            }
        });
    }); };
    UserController.update = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var user, id, _b, email, password, userRepository, error_6, validationOpt, errors, error_7;
        return __generator(_a, function (_c) {
            switch (_c.label) {
                case 0:
                    id = req.params.id;
                    _b = req.body, email = _b.email, password = _b.password;
                    userRepository = data_source_1.AppDataSource.getRepository(User_1.User) //conexion a la base de datos
                    ;
                    _c.label = 1;
                case 1:
                    _c.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, userRepository.findOneByOrFail({ id: parseInt(id) })]; //BUSCAMOS AL USUARIO
                case 2:
                    user = _c.sent(); //BUSCAMOS AL USUARIO
                    user.email = email;
                    user.password = password;
                    return [3 /*break*/, 4];
                case 3:
                    error_6 = _c.sent();
                    return [2 /*return*/, res.status(404).json({
                            message: "User not found",
                            error: error_6.message
                        })];
                case 4:
                    validationOpt = { validationError: { target: false, value: false } };
                    return [4 /*yield*/, (0, class_validator_1.validate)(user, validationOpt)];
                case 5:
                    errors = _c.sent();
                    if (errors.length > 0) {
                        return [2 /*return*/, res.status(400).send(errors)];
                    }
                    _c.label = 6;
                case 6:
                    _c.trys.push([6, 8, , 9]);
                    return [4 /*yield*/, userRepository.save(user)];
                case 7:
                    _c.sent();
                    return [3 /*break*/, 9];
                case 8:
                    error_7 = _c.sent();
                    return [2 /*return*/, res.status(500).json({
                            message: "Error Updating User",
                        })];
                case 9: return [2 /*return*/, res.status(201).json({
                        message: "Update User",
                    })];
            }
        });
    }); };
    return UserController;
}());
exports.UserController = UserController;
//# sourceMappingURL=UserController.js.map